 ...
 * lispconf.c
 *
 * Lisp user-level configuration and querying utility. Communicates
 * with the daemon using a domain socket.
 *
...
{
    lisp_cmd_t *cmd = malloc(MAX_MSG_LENGTH);
...
int process_list_responses(void)
{
    ...
    while (1) {
        if ((retval = recv(dsock_fd, cmd, MAX_MSG_LENGTH,  0)) < 0) {
            perror("recv");
            break;
        }
...
int set_rloc_interface(struct gengetopt_args_info *args_info)
{
    ...
    memset((char *)&rloc_msg, 0, sizeof(lisp_set_rloc_msg_t) + sizeof(rloc_t));

    rloc_t *rloc = (rloc_t *)rloc_msg->rlocs;
    rloc_msg = (lisp_set_rloc_msg_t *)cmd->val;
    rloc->addr.address.ip.s_addr = ((struct sockaddr_in *)&ifr.ifr_addr)->sin_addr.s_addr;

    retval = send_command(cmd, cmd->length + sizeof(lisp_cmd_t));
...
int main(int argc, char **argv) 
{
    ...
    /*
     * Connect to the lispd daemon
     */
    memset((char *)&client_sock, 0 ,sizeof(struct sockaddr_un));

    if (make_dsock_addr(LispClientIPCFile, &client_sock) < 0) {
            printf("make_dsock failed");
            return 0;
    }

    if (make_dsock_addr(LispServerIPCFile, &server_sock) < 0) {
        printf("make_dsock faile");
        return 0;
    }

    if ((dsock_fd = socket(AF_UNIX, SOCK_DGRAM, 0) ) < 0) {
        perror("socket");
        exit(-1);
    }

    unlink(LispClientIPCFile);
    if (bind(dsock_fd, (struct sockaddr *)&client_sock, sizeof(struct sockaddr_un)) < 0 ) {
