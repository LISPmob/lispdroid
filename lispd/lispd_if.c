 ...
 * create_loopback()
 *
 * Creates a loopback interface with the specified address and
 * name. Uses the old ioctl, SIOCGIFFLAGS system for now,
 * there is little documentation on adding an intf with netlink
 */
int create_loopback(lisp_addr_t *addr, char *name)
{
    struct ifreq ifr;
    struct sockaddr_in *sp;
    int    netsock;
